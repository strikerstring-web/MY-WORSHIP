...
const MyAccount: React.FC<MyAccountProps> = ({ state, setCurrentView, t, onLogout, toggleTheme, toggleNotifications, updatePrayerReminder }) => {
  const stats = useMemo(() => {
  ...
  return (
    <div className="scroll-container px-6 pt-12 content-limit w-full">
      ...
      <div className="flex items-center justify-between mb-10 animate-fade-up">
         <div>
            <h1 className="text-3xl font-black text-white tracking-tighter">{state.profile.name}</h1>
      ...
      <div className="grid grid-cols-2 gap-4 mb-12 animate-fade-up stagger-2">
         <div className="card-premium flex flex-col items-center !p-6">
            ...
            <p className="text-2xl font-black text-white">{stats.completedFard}</p>
         ...
         <div className="card-premium flex flex-col items-center !p-6">
            ...
            <p className="text-2xl font-black text-white">{stats.missedFard}</p>
             ...
             return (
               <div key={p} className="card-premium flex items-center justify-between !p-5 group">
                 <div className="flex items-center gap-5">
                   <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg ${reminder.enabled ? 'bg-emerald-600 text-white' : 'bg-white/5 text-slate-600'}`}>
                     <i className="fas fa-bell"></i>
                   </div>
                   <div>
                     <span className="font-black text-sm text-white capitalize">{t(p)}</span>
                     <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest mt-0.5">Custom Reminder</p>
                   </div>
                 </div>
                 <div className="flex items-center gap-4">
                   <input 
                     type="time" 
                     className="bg-transparent border-none text-xs font-black text-emerald-500 focus:ring-0 cursor-pointer" 
                     value={reminder.time} 
                   ...
                   <button 
                     ...
                     className={`w-12 h-6 rounded-full p-1 transition-all ${reminder.enabled ? 'bg-emerald-600' : 'bg-white/10'}`}
                   >
      ...
      <div className="space-y-6 animate-fade-up stagger-4 pb-20">
         ...
         <button onClick={toggleNotifications} className="card-premium w-full flex items-center justify-between !p-6">
            <div className="flex items-center gap-5">
               <div className="w-12 h-12 bg-emerald-500/10 text-emerald-500 rounded-2xl flex items-center justify-center text-xl"><i className="fas fa-shield-halved"></i></div>
               <div className="text-left">
                 <span className="font-black text-sm text-white">Global Notifications</span>
                 <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest mt-0.5">Worship Status Sync</p>
               </div>
            </div>
            <div className={`w-12 h-6 rounded-full p-1 transition-colors ${state.settings.notificationsEnabled ? 'bg-emerald-600' : 'bg-white/10'}`}>
